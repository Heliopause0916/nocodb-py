...
1. **单行文本 (SingleLineText)**
   - 验证：字符串类型或 null，无长度限制
   - 转换：直接使用字符串值，null 值保持不变
   - 元数据：无特殊配置

2. **长文本 (LongText)**
   - 验证：字符串类型或 null，支持富文本模式
   - 转换：直接使用字符串值，null 值保持不变
...
4. **小数类型 (Decimal)**
   - 验证：小数类型或 null，支持精度控制
   - 转换：转换为浮点数，null 值保持不变
...
#### 时间格式设置的重要发现 ✅ 已验证
基于时间格式测试数据的分析，发现关键结论：

**前端格式设置不影响后端存储值**：尽管不同的时间列设置了不同的显示格式（如 `YYYY-MM-DD`、`YYYY/MM/DD`、`DD-MM-YYYY`、`YYYY-MM` 等），但后端存储的实际值完全一致。

**统一的后端存储格式**：
- **日期时间**：`"2025-12-28 16:00:00+00:00"`（ISO 8601格式，带时区）
- **日期**：`"2025-12-29"` 和 `"2025-12-01"`（YYYY-MM-DD格式）
- **时间**：`"21:51:00"`（HH:mm:ss格式）

**对SDK设计的启示**：
1. **验证器设计**：时间相关列的验证器只需要验证标准的后端存储格式，无需考虑前端显示格式
2. **API接受类型**：SDK应该接受标准的时间格式，NocoDB后端会自动处理格式转换
3. **Python推荐类型**：可以使用Python的datetime对象，SDK负责转换为标准字符串格式
...
*创建时间：2025-12-21*
*最后更新：2025-12-29*
*更新内容：时间格式设置测试发现 - 前端格式不影响后端存储值，验证器设计优化*
